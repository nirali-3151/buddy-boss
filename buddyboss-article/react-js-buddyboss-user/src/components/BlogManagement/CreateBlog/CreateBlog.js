...
class CreateBlog1 extends React.Component {
  ...
  onClickPublishBtn = async () => {
    ...
    // var time = moment(created_at).format('DD MMM YYYY')  + created_at.getHours() + ":" + created_at.getMinutes() + ":" + created_at.getSeconds();
    // console.log("time is : " ,time);

    // const created_at1 = moment(created_at).format('DD MMM YYYY')
    ...
    if (thumb_nail_image === "" || title === "" || desc === "" || blog_catagory_id === "" || radio_btn_select === "") {
      ...
      const initialData = await UserService.thumbNailImageAddList(formData);
    }
  }
