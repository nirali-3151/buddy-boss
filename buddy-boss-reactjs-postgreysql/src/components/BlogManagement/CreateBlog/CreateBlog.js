...
import {AddBlogManagementData} from "../../../reduxStore/Actions/BlogManagement"
...
class CreateBlog1 extends React.Component {
  ...
  changeValueOfSingleQuateDescription = () => {
    ...
    const replaced = desc.replaceAll("'", "''");
  ...
  getDataOfMainBlog = async () => {
    ...
    const initialData = await UserService.getMainBlogOfUserList(Token);
  ...
  onClickPublishBtn = async () => {
    ...
    if (blogData > 0) {
      ...
      const initialData = await UserService.UpdateMainBlogOfUserList(token);
    }
    ...
    if (thumb_nail_image === "" || title === "" || desc === "" || blog_catagory_id === "" || radio_btn_select === "") {
      ...
      // formData.append('blog_catagory_id', blog_catagory_id)
      ...
      const initialData = await UserService.thumbNailImageAddList(token, newData);
      this.props.AddBlogManagementData1({blog_manage_Data:newData})
    }
  }
  ...
  render() {
    ...
    console.log("thumbnail img : " ,thumb_nail_image );
    const { previewImgFlag } = this.state

    const {count} = this.props.FirstPage
    // console.log("blog_manage_Data",blog_manage_Data);
...
const mapDispatchToProps = (dispatch) => {
  return {
    AddBlogManagementData1: (payload) => dispatch(AddBlogManagementData(payload))
